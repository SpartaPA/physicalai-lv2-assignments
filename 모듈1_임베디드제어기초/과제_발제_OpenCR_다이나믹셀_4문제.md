...
전체 4문제 완료 시 `lv2_module1_이름.zip`에 답안과 실행 A·B의 로그 또는 캡처를 제출합니다. 답안은 표와 구조도를 포함해 2쪽 이내를 권장합니다. 모델·ID·게인·목표각·속도 상한·예제 이름을 적고, 수정한 코드가 있다면 변경 파일만 추가하세요. 오늘 문제 1만 제출할 때는 별도 발제문서의 제출 형식을 따릅니다.
